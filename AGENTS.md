...
Personal agent skills collection. Generate, sync, or hand-maintain [Agent Skills](https://agentskills.io/home) for use in AI-assisted development.
...
- **Active:** vuejs-ai (vue-best-practices, vue-debug-guides, vue-pinia-best-practices, vue-router-best-practices, vue-testing-best-practices, create-adaptable-composable)
- **Planned:** VueUse
...
For skills that are manually maintained, including original skills and forks from external sources that are modified locally. These skills cannot be synced because local modifications would be overwritten.
...
│       ├── GENERATION.md       # Tracking metadata (for Type 1 generated skills)
│       ├── SYNC.md             # Tracking metadata (for Type 2 synced skills)
│       ├── LICENSE             # License file
│       ├── NOTICE.md           # Provenance and attribution
│       └── references/
│           └── *.md            # Individual skill files
│
├── legal/                      # Legal tracking
│   └── sources.yml             # Upstream source metadata
│
└── scripts/                    # Tooling
    └── validate-skills.mjs     # Skill validation script
```

**Important:** For Type 1 (generated), the `skills/{project}/` name must match `sources/{project}/`. For Type 2 (synced), the output name is configured in `meta.ts` and may differ from the source skill name. Type 3 (hand-written) skills have no external source link and need neither `GENERATION.md` nor `SYNC.md`.
...
2. **Add the git submodule:**
   ```bash
   git submodule add https://github.com/org/repo sources/new-project
   ```
...
- **Read** source docs from `sources/{project}/`
...
Hand-written skills are maintained directly in `skills/{name}/`. They may originate from external sources (forks), but are now independently maintained.

- Keep `NOTICE.md` to document provenance if the skill was forked from elsewhere.
- Keep `LICENSE` from the original source for legal compliance.
- No `GENERATION.md` or `SYNC.md` is needed.
